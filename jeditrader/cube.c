...
static void init_buffers(Window* window) {
  ...
  window->chart->cube_transforms = glMapBufferRange(GL_ARRAY_BUFFER, (GLintptr)NULL, num_bytes, flags);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    glVertexAttribPointer(loc, 4, GL_FLOAT, GL_FALSE, 4 * 4 * sizeof(float), offset);
  ...
  glBufferStorage(GL_ARRAY_BUFFER, num_bytes, NULL, flags);
  window->chart->cube_colors = glMapBufferRange(GL_ARRAY_BUFFER, (GLintptr)NULL, num_bytes, flags);
...
void cube_init(Window* window) {
  ...
  window->cube.uni_world = glGetUniformLocation(window->cube.program, "gWorld");
  init_buffers(window);
}
...
void cube_render_frame(Window* window, u32 num_cubes) {
  ...
  glBindVertexArray(window->cube.vao);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, window->cube.ebo);
  glUniformMatrix4fv(window->cube.uni_world, 1, GL_FALSE, &window->chart->g_world.data[0][0]);

  glDrawElementsInstanced(GL_TRIANGLE_STRIP, sizeof(indices) / sizeof(GLuint), GL_UNSIGNED_INT, NULL, num_cubes);
