...
static void init_buffers(Window* window, Axes* axes) {
	...
	glBufferData(
	 GL_ARRAY_BUFFER, sizeof(axes->vertices), axes->vertices, GL_DYNAMIC_DRAW);
	...
	glVertexAttribPointer(
	 1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)offset);
}
...
void axes_init(Window* window, Axes* axes) {
	...
	window->axes.uni_world =
	 glGetUniformLocation(window->axes.program, "gWorld");
...
void axes_render_frame(Window* window, Axes* axes) {
	...
	glBufferData(
	 GL_ARRAY_BUFFER, sizeof(axes->vertices), axes->vertices, GL_DYNAMIC_DRAW);
	glUniformMatrix4fv(
	 window->axes.uni_world, 1, GL_FALSE, &window->chart->g_world.data[0][0]);
	glDrawArrays(GL_LINES, 0, sizeof(axes->vertices) / sizeof(float));
}

static vec3 line_plane_collision(
 vec3 plane_norm,
 vec3 plane_pnt,
 vec3 ray_dir,
 vec3 ray_pnt) {
...
static vec3 world_coords_xyz(Window* window, Cam* cam) {
	...
	vec4 ray_eye =
	 mat4_mult_vec4(mat4_inv(window->chart->perspective), ray_clip);
	ray_eye.z = 1.0f;
	ray_eye.w = 0.0f;
	...
	vec3 xyz_point = line_plane_collision(
	 Vec3(0, 0, 1), Vec3(0, 0, 0), ray_world.xyz, cam->eye);
	xyz_point.x *= -1;
...
void axes_update(Window* window, Axes* axes) {
	bool button1_last = window->mouse_last[SDL_BUTTON_LEFT];
	bool button1_cur = window->mouse_cur[SDL_BUTTON_LEFT];
	if (button1_last != button1_cur) {
		vec3 xyz_point = world_coords_xyz(window, &window->chart->cam);
		if (!axes->selecting && button1_cur == 1) {
			axes->selecting = true;
			axes->sel_start = xyz_point.xy;
		} else if (button1_cur == 0) {
			sel_reset(axes);
			sel_write_vertices(axes);
		}
	}
